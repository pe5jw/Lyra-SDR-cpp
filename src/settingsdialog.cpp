...
QWidget *SettingsDialog::buildHardwareTab() {
    ...
    if (stream_) {
        ...
        // Mic Boost (+20 dB hardware) is exposed as a lit chip on
        // the TX panel itself (next to the Mic Gain slider) so the
        // operator can flip it during a session — voice-chain
        // adjustment, not a one-time setup item.  See TxPanel.qml.
