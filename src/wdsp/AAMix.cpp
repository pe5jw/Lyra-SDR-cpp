...
// Stage B.6.b-debug instrumentation REMOVED in B.6.b-final
// (2026-06-08, commit 8b8e0da was the root-cause fix).  The 9-counter
// chain-side scaffolding + ENTRY beacon + 1-Hz log lines proved AAMix
// runs end-to-end at 188 frames/sec on the operator's HL2+ -- but
// then localised the silent-audio defect to the dispatch side
// (counters healthy / [disp-dbg] silent), which led to the Stage-A
// `SendpOutboundRx` no-op stub at `RadioNet.cpp:272` overwriting
// `aaMix_->Outbound` ~1 ms after `openRx1` wired the real lambda.
// The fix (removing that stub) is preserved in B.6.b-fix1.  Keeping
// the debug atoms + qInfo lines would flood the log 4 lines/sec
// forever for zero ongoing value, so they're stripped here.  If a
// future regression suspects AAMix internals, restore the scaffold
// from commit `225518c` (Stage B.6.b-debug).
