...
// Source file: ChannelMaster/aamix.c (whole file, verbatim)
// Source version: 2.10.3.13 (MI0BOT HL2 fork)
// Original copyright: (C) 2014 Warren Pratt, NR0V
// License: GNU General Public License v2 or later
//
// DIRECT PORT 2026-06-11 (P0.c).  See AAMix.h preamble for the
// attribution + the packaging-only differences.  Two notes:
//   * The reference aamix.c includes only cmcomm.h (which carries
//     windows/process/intrin/math/time/avrt + the family headers);
//     Lyra-cpp's cmcomm.h carries the comm surface and the WDSP
//     resampler entry points resolve through wire/wdspcalls.h (the
//     operator-approved seam) instead of a static link — call
//     sites read byte-identical.
//   * `AvSetMmThreadPriority(hTask, 2)` carries an explicit
//     (AVRT_PRIORITY) cast — C++ refuses the C int→enum implicit
//     conversion; the value (2 = AVRT_PRIORITY_HIGH) is verbatim.

#include "wire/cmcomm.h"
#include "wire/AAMix.h"
...
void mix_main (void *pargs)
{
	...
	if (hTask != 0) AvSetMmThreadPriority(hTask, (AVRT_PRIORITY) 2);
...
{
	// (C4189 suppressed: the reference keeps `handle` for its
	// commented-out SetThreadPriority line — verbatim text wins.)
