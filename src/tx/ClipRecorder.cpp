...
void ClipRecorder::start(Source src, int maxSamples) {
    std::lock_guard<std::mutex> lk(mtx_);
    buf_.clear();
    buf_.reserve(48000 * 8);            // ~8 s headroom; grows as needed
    source_ = src;
    limit_.store(std::clamp(maxSamples, 1, kMaxSamples), std::memory_order_relaxed);
...
void ClipRecorder::feedMicPairs(const double *iq_pairs, int n_pairs) {
    ...
    const std::size_t cap = static_cast<std::size_t>(limit_.load(std::memory_order_relaxed));
    for (int i = 0; i < n_pairs && buf_.size() < cap; ++i)
        buf_.push_back(static_cast<float>(iq_pairs[2 * i]));   // I = mic; Q ignored
    size_.store(static_cast<int>(buf_.size()), std::memory_order_release);
}
...
void ClipRecorder::feedRxMono(const float *mono, int n) {
    ...
    const std::size_t cap = static_cast<std::size_t>(limit_.load(std::memory_order_relaxed));
    for (int i = 0; i < n && buf_.size() < cap; ++i)
...
void ClipRecorder::feedRxStereoDup(const double *audio, int nframes) {
    ...
    if (!recording_.load(std::memory_order_relaxed) || source_ != Source::Rx) return;
    const std::size_t cap = static_cast<std::size_t>(limit_.load(std::memory_order_relaxed));
    for (int f = 0; f < nframes && buf_.size() < cap; ++f)
