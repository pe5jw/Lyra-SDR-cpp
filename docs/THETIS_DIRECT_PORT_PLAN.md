...
| **P3**  | [WIP]      | netInterface registrations + obbuffs ring lifecycle.  **SHIPPED 2026-06-12, HL2 bench PENDING** — create_rnet moved to once-per-process at the main.cpp QTimer AFTER create_xmtr (the reference C#-init order; fixes the per-open prn re-allocation leak); SendpOutboundTx(OutBound) restored at the reference site (netInterface.c:1761, tail of create_rnet); UpdateRadioProtocolSampleSize verbatim (netInterface.c:1836-1858, diff IDENTICAL) called per-open per StartAudio:45 — creates obbuffs rings 0/1 (2 new ob_main pump threads per session, idle: no producers until P4); destroy_obbuffs(0/1) at close() per StopAudio:112-113.  RX side deliberately NOT registered (WdspEngine hybrid owns RX dispatch until P4 — the B.6.b clobber class). | **OPERATOR HL2 BENCH GATE**: RX works; multiple stop/start cycles clean; clean app shutdown (ob threads join via destroy_obbuffs). |
